...
-----------
-- Paketti Stem Loader
-- Loads multiple audio files (stems), each into its own Pakettified instrument on its own track,
-- with C-4 + 0G01 triggers, *Instr. Macros devices, and automatic BPM detection from file metadata.
-----------
...
-- Create pattern sequence based on the longest loaded sample
...
function pakettiStemLoader(normalize)
  ...
  -- Create ProcessSlicer for the loading operation
  stem_loader_progress.slicer = ProcessSlicer(pakettiStemLoader_process, selected_sample_filenames, normalize)
...
function pakettiStemLoader_process(selected_sample_filenames, normalize)
  -- Completely stop AutoSamplify monitoring to prevent double-processing
  local AutoSamplifyMonitoringState = PakettiTemporarilyDisableNewSampleMonitoring()
  ...
  for index, filename in ipairs(selected_sample_filenames) do
    ...
    local next_instrument = song.selected_instrument_index + 1
    ...
    -- 3. Load the default XRNI preset (Pakettification)
    pakettiPreferencesDefaultInstrumentLoader()
    ...
    if selected_instrument.samples[1].sample_buffer:load_from(filename) then
      ...
      -- 6. Apply loader settings - FORCE autoseek ON (hardcoded)
      current_sample.interpolation_mode = preferences.pakettiLoaderInterpolation.value
      current_sample.oversample_enabled = preferences.pakettiLoaderOverSampling.value
      current_sample.autofade = preferences.pakettiLoaderAutofade.value
      current_sample.autoseek = true  -- ALWAYS ON for stems
      current_sample.loop_mode = preferences.pakettiLoaderLoopMode.value
      current_sample.oneshot = preferences.pakettiLoaderOneshot.value
      current_sample.new_note_action = preferences.pakettiLoaderNNA.value
      current_sample.loop_release = preferences.pakettiLoaderLoopExit.value
      ...
      -- 8. Write C-4 + 0G01 to line 1 of pattern 1
      ...
      if pattern and pattern.tracks[new_track_index] then
        ...
        if song.tracks[new_track_index].visible_effect_columns < 1 then
          song.tracks[new_track_index].visible_effect_columns = 1
        end
        
        print(string.format("Stem Loader: Wrote C-4 (instrument %02X) + 0G01 to Track %d, Pattern 1, Line 1", 
          instrument_value, new_track_index))
      end
      
      -- 9. Load *Instr. Macros device on the track
      if song.tracks[new_track_index].type ~= 2 then  -- Not master track
        loadnative("Audio/Effects/Native/*Instr. Macros", nil, nil, nil, true)
  ...
  -- 13. Create pattern sequence based on longest sample
