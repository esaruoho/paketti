...
**Several samples on one key.** Renoise lets samples share a key, and the export keeps that: one pad per Renoise note, laid out from C1 upward. Eight kicks on C-0 and eight snares on C#0 become two pads of eight, not sixteen pads. A pad with one sample is a Simpler; a pad with several is a Sampler with one zone per sample, velocity ranges included.
...
- **Keep this instrument's key layout** - on by default whenever the instrument actually has a layout, meaning its samples sit on distinct narrow key ranges. A General MIDI drum kit, a multisample, anything you have deliberately mapped exports exactly where you put it. Turn it off and the samples are laid out one per key from C-3 instead, which is what an unmapped pile of samples wants. The dialog says which of the two you are getting, and tells you when it spots General MIDI drum keys.
- **Name waves after the General MIDI drum on each key** - if your kit is laid out on GM percussion keys, which is what Renoise's own GM kit template does, the exported waves get named for what they are (`KICK1`, `SNARE1`, `HHCLOSED`) instead of `DRUM_017`. This switches itself on when three or more samples are already sitting on GM drum keys. Renoise's note numbers are the same as MIDI's, so the GM kick is the row Renoise labels C-3. Eight characters is all the sampler's screen shows, so this makes a real difference when browsing on the machine.
...
**Loading a kit that spans disks.** Load disk 1 and the sampler will ask for the rest as it needs them, showing which one: `Missing wave VIBSLAP (from KITNAME2)`. That is a request, not an error - swap to that disk image while the message is on screen and press ENTER, and the prompt withdraws itself as soon as the wave is found. Press Skip only if you genuinely want to leave a sample out. The disk name in parentheses is there because Paketti writes it into every reference; without it the sampler can only say something is missing, not where it lives.

**When the kit spans more than one disk, the disks are chained.** The voices, the performance and an `.X01` setup all sit on disk 1, the waves are packed across the set, and every wave reference names the disk that wave landed on - so the sampler fetches the later disks itself as it needs them. Load disk 1, pick the `.P01`, and feed it the next disk when it asks. The performance addresses every voice on MIDI channel 1 and channel 10, and the voices cover disjoint key ranges, so it is one kit across the whole keyboard. The output folder is created if it does not exist yet. Pick a folder and you get `KITNAME_DISK1.img`, `KITNAME_DISK2.img` and so on, plus a `files` subfolder with the waves, voice, and performance loose if you would rather assemble a disk yourself.
...
**Find Missing Plugins... / Remove Missing Plugins** (`Main Menu:Tools:Paketti:Plugins/Devices:`, also Mixer and DSP Chain right-click) - finds VST/VST3/AU/LADSPA/DSSI effects a song refers to that aren't installed any more and left dead placeholders in the chains. Checks track DSP chains and sample FX chains (both cleanable) and instrument plugins (reported only, since removing one would throw away the instrument's settings). Native, routing and meta devices are never considered. `Find` shows a report grouped by track with a Remove These N button; `Remove` does it straight away. One undo step.

**Find Multi-Instrument Columns** (`Main Menu:Tools:Paketti:Pattern Editor:`) - finds note columns playing more than one instrument and jumps to them, scanning the pattern sequence in playback order so hits arrive in the order you'd hear them. Next/Previous/Rescan all have their own shortcuts that work without the dialog open and wrap around at the ends; the dialog itself has a button to split the track you just landed on. Pairs with Split Track by Instrument.

**Spread Round Robins by Velocity** (`Main Menu:Tools:Paketti:Instruments:`, Sample Mappings and Instrument Box right-click, and a button in the Keyzone Distributor) - when several samples sit on the SAME key, that's a round robin, and this splits each such group into its own velocity bands independently of the other groups. Four snares on C-4 and three hats on F#4 each get their own spread in one pass. Eight or fewer uses the fixed 71-7F / 61-70 / ... layout with the lowest stretched to 00; more than eight divides 0-127 evenly. Turns VEL->VOL off, because in a round robin velocity picks which sample plays rather than how loud it is. Keys with a single sample are untouched, and read-only slice mappings are skipped.

**Split Selected Track by Instrument** (`Main Menu:Tools:Paketti:Pattern Editor:`, also in the Pattern Editor `Tracks` and Mixer right-click menus) - a track with instruments 00, 03 and 07 jumbled together becomes three tracks, one per instrument, inserted right after it. Different from **Explode Notes to New Tracks**, which splits on pitch (one track per note, for drum kits) - this splits on instrument. New tracks inherit the source's DSP chain, colour and column visibility so nothing changes how it sounds, and volume/panning/delay/sample-effects travel with each note. Notes with no instrument number follow the last instrument in that column, and track effect commands follow the leftmost instrument on their line. Works across every pattern in the song. The plain version is non-destructive - it keeps the source track untouched and just mutes its note columns, so nothing double-triggers and unmuting gets you the original back; `(Delete Source Track)` removes it instead. There's a `Split All Tracks by Instrument` for the whole song.
