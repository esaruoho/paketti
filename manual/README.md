...
**When the kit spans more than one disk, every reference names the diskette its wave landed on.** That is what makes the sampler ask for disk 2 by name instead of just reporting a wave missing with no idea where it lives. The `.X01` setup ships as well, listing all of it in one place. The output folder is created if it does not exist yet. Pick a folder and you get `KITNAME_DISK1.img`, `KITNAME_DISK2.img` and so on, plus a `files` subfolder with the waves, voice, and performance loose if you would rather assemble a disk yourself.
