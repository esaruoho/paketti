...
**Slice loops.** If your slices loop — Wipe&Slice can set that on every slice it makes — the loops travel with them. Simpler stores its loop on the device, so Paketti writes the loop toggle and the fraction of the pad's region the loop covers, and reads the same back on import. Simpler only loops forwards, so Renoise's reverse and ping-pong slices arrive in Live as plain forward loops.
