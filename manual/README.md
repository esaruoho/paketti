...
**When the kit spans more than one disk, every disk is self-contained.** Each one gets a voice, a performance and exactly the waves that voice plays, so it loads on its own with nothing missing - load them in any order and each disk's pads join the ones already in memory. A 120-pad kit is three voices of 40 splits, so it is three disks; all their performances answer on MIDI channel 1 and channel 10, and their key ranges are disjoint, so the full kit is playable once they are all in. Disk 1 also carries an `.X01` setup cataloguing the lot. The output folder is created if it does not exist yet. Pick a folder and you get `KITNAME_DISK1.img`, `KITNAME_DISK2.img` and so on, plus a `files` subfolder with the waves, voice, and performance loose if you would rather assemble a disk yourself.
