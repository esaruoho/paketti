...
# Phrase Presets: Batch Import and Export

**Source:** `PakettiMidiImport.lua`

Renoise saves a single phrase as a `.xrnz` preset. These four commands move whole folders of them, and unlike PhraseMate they look inside subfolders.

All of them live under **`Tools > Paketti > Instruments`**.

## Export

| Command | What it writes |
|---|---|
| `Save All Phrases as Presets (.xrnz)...` | every phrase of the **selected** instrument into one folder you pick, as `01_PhraseName.xrnz`, `02_...` |
| `Save Phrases of All Instruments as Presets (Subfolders)...` | every phrase of **every** instrument in the song, one subfolder per instrument: `1A_InstrumentName/01_PhraseName.xrnz` |

The destination folder is created if it does not exist. If it cannot be created, or a write fails, you get one message and it stops — rather than a modal error for every remaining phrase.

## Import

| Command | What it does |
|---|---|
| `Load All Phrase Presets from Folder (.xrnz)...` | loads every `.xrnz` in the folder **and all its subfolders, at any depth** into the **selected** instrument, in path order |
| `Load Phrase Presets from Folder Tree (One Instrument per Subfolder)...` | walks the tree and gives **each folder its own instrument**, named after that folder |

The second one is the mirror image of `Save Phrases of All Instruments`, so an exported tree comes back as the instruments it came from instead of collapsing into one. It is also the one to use for large libraries: an instrument holds at most 126 phrases, and a folder with more than that spills into `FolderName (2)`, `(3)` and so on, so nothing in the tree is silently dropped.

Example — this tree:

```
MyPhrases/root1.xrnz
MyPhrases/Bass/b1.xrnz
MyPhrases/Bass/b2.xrnz
MyPhrases/Drums/d1.xrnz
MyPhrases/Drums/kick/k1.xrnz
MyPhrases/Drums/kick/k2.xrnz
```

imports as four instruments: `Bass` (2 phrases), `Drums` (1), `kick` (2), `MyPhrases` (1).

## Keybindings and MIDI

- Keybinding: `Global:Paketti:Save All Phrases as Presets (.xrnz)...`
- Keybinding: `Global:Paketti:Save Phrases of All Instruments as Presets (Subfolders)...`
- Keybinding: `Global:Paketti:Load All Phrase Presets from Folder (.xrnz)...`
- Keybinding: `Global:Paketti:Load Phrase Presets from Folder Tree (Per Subfolder)...`
- MIDI Mapping: `Paketti:Save All Phrases as Presets (.xrnz)... x[Button]`
- MIDI Mapping: `Paketti:Load All Phrase Presets from Folder (.xrnz)... x[Button]`
...
Amigo's slices come back too, including any slicing you did inside Amigo. A round trip is frame-exact: a 35-slice RX2 sent to Amigo and pulled back gives 35 Renoise slice markers on precisely the same frames.
...
Counts stop at 64 because that is how many slice slots Amigo has, and 64 fits exactly: Amigo's `slice0` is the sample start, so a Renoise marker sitting on frame 1 is not written into a slot of its own.

- Menu: `Main Menu:Tools:Paketti:Instruments:Amigo:Wipe&Slice&Amigo (002 / 004 / 008 / 016 / 032 / 064)`
- Menu: the same six under `Instrument Box:Paketti:Amigo:` and `Sample Editor:Paketti:Amigo:`
- Keybinding: `Global:Paketti:Wipe&Slice&Amigo (002)` … `(064)`
- MIDI Mapping: `Paketti:Wipe&Slice&Amigo (002)` … `(064)`
...
**Every Amigo in Song to Renoise Instruments** walks the song, and for each instrument holding Amigo it builds a sampled Renoise instrument with that audio and those slice points. A song of 10 Amigos ends up as those 10 Amigos followed by 10 sampled instruments with the slices set. The Amigo instruments are not touched — their plugin state is byte-identical afterwards.

**Every Sampled Instrument in Song to Amigo** is the mirror image: every instrument that has sample data and does not already host Amigo gets an Amigo built for it, appended in order.

Exported wavs never overwrite each other. If two samples share a name the second becomes `name-2.wav`, the third `name-3.wav`, and an existing file in the folder is never clobbered. The one exception is re-exporting a sample into an Amigo that already points at that file, which lands on the same file instead of piling up copies.
...
- Menu: `Instrument Box:Paketti:Amigo:Batch: …` (both)
...
|--------|-------------------|
| `.rx2` | drag and drop, or the Paketti Import menu |
| `.rex` | drag and drop, or the Paketti Import menu |
| `.pti` | drag and drop, or the Paketti Import menu |
| `.iti` | drag and drop, or the Paketti Import menu |
| `.ot` | drag and drop, or the Paketti Import menu |
...
- Menu: `Main Menu:Options:Sliced Imports Also Go Into Amigo (RX2, REX, PTI, ITI, OT, WAV+CUE) Toggle`
...
⌨ **10,979** keybindings · 🎛 **7,565** midimappings · ☰ **6,883** menu entries → **4,413** functions
<!-- /AUTO -->
