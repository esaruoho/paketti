...
**The whole RX2 decoder layer is now in one place.** There were *four* copies of the Linux branch — the single-file import, both batch RX2 converters, and a byte-identical duplicate of the entire OS-setup function inside `PakettiPolyendSuite.lua` (used by RX2 to PTI conversion). That last one meant a Wine fix could land in the RX2 loader and silently miss the Polyend path. All of it now lives in `PakettiRX2Loader.lua` behind four shared entry points — `PakettiRX2SetupDecoderPaths` (decoder location, macOS quarantine/chmod, Linux Wine pre-flight), `PakettiRX2BuildDecoderCommand`, `PakettiRX2TempFolder`, `PakettiRX2WineAvailable` — and `PakettiPolyendSuite.lua` calls them instead of carrying its own copy.

The shell-quoting fix went to every shell-out in the tool, not just RX2: `pakettiFSPath.shell_quote()` is now shared, and `PakettiHack.lua` (unzip/zip of a temporary `.xrni`) and `PakettiXRNSProbe.lua` (plutil) use it too.
...
**New: Direct Send.** Keep Source still creates the Send device silent at -inf dB, so you dial the amount in by hand. Direct Send is the same thing at 0dB unity — source kept, send immediately audible, nothing to reach for. Mute Source is also at 0dB.

So the three modes are: **Keep Source** = source kept, send silent · **Direct Send** = source kept, send at 0dB · **Mute Source** = source muted, send at 0dB.

Create New Multiband Send Track gets all of the above too, and two fixes of its own: all three bands now feed the new send track (previously only the first band was pointed at it), and Mute Source mode sets the per-band Mute Source switches — which the Multiband Send device does have, despite an old code comment claiming otherwise.

- New keybindings: `Global:Paketti:Create New Send Track (Direct Send) (Preset++)`, `Global:Paketti:Create New Multiband Send Track (Direct Send) (Preset++)`
- Existing keybindings: `Global:Paketti:Create New Send Track (Keep Source) (Preset++)`, `Global:Paketti:Create New Send Track (Mute Source) (Preset++)`, and the Multiband equivalents
- New menu entries: `Mixer:Paketti:Preset++:Create New Send Track (Direct Send)` and `Create New Multiband Send Track (Direct Send)`, plus the same pair under DSP Chain, DSP Device, Pattern Matrix and Pattern Editor
...
### 2026-07-22 - Feature: Step Mute (Non-Destructive) — gate tracker steps by flipping volume, notes kept
A new way to gate parts of a melody/arp on and off from a MIDI controller (LPD8, APC, etc.) **without destroying the notes**. Where the Groovebox 8120 step toggles delete/rewrite notes, Step Mute instead flips the note-column **volume** of a line to 0 and remembers the original — toggle a step off and it goes silent, toggle it back on and the exact original volume is restored (an empty/unset volume returns to full). Your 8-note arp keeps all its notes; you just gate bits of it in and out. It works on the **selected track only**. A **dynamic sliding window** maps N MIDI buttons (N = 8/16/32/64, your choice) to N consecutive lines: the window follows the **playhead while playing** and the **edit cursor when stopped**, so an LPD8's 8 pads always address the 8 lines "where you are", and as playback moves into the next chunk the pads automatically re-map to those lines. A dialog shows the current window as clickable step buttons (green = playing, red = muted, brighter = the playhead line) and follows along live. New menu entries: `Main Menu:Tools:Paketti:Sequencer:Step Mute (Non-Destructive)...`, `Pattern Editor:Paketti:Step Mute:Show Step Mute Dialog...`, `Pattern Editor:Paketti:Step Mute:Toggle Mute Current Row`, `Pattern Editor:Paketti:Step Mute:Unmute All in Track`. New keybindings: `Global:Paketti:Step Mute Show/Hide Dialog`, `Global:Paketti:Step Mute Toggle Current Row`, `Global:Paketti:Step Mute Unmute All in Track`, `Global:Paketti:Step Mute Window Previous`, `Global:Paketti:Step Mute Window Next`, and `Global:Paketti:Step Mute Window Size 8/16/32/64`. New MIDI mappings: `Paketti:Step Mute Toggle Step 01`…`64 [Trigger]` (window-relative step buttons — map your pads to Steps 01-08 for an LPD8), `Paketti:Step Mute Show/Hide Dialog [Trigger]`, `Paketti:Step Mute Toggle Current Row [Trigger]`, `Paketti:Step Mute Unmute All in Track [Trigger]`, `Paketti:Step Mute Window Previous/Next [Trigger]`, `Paketti:Step Mute Set Window Size 8/16/32/64 [Trigger]`, and `Paketti:Step Mute Set Window Size x[Knob]`. **Per-controller banks** (so you can have several controllers plugged in at once and map each one's pads to its own independent set of step buttons — all driving the same shared window): `Paketti:Step Mute LPD8 Step 01`…`08 [Trigger]` (8 pads), `Paketti:Step Mute MidiMix Step 01`…`16 [Trigger]` (16 buttons), `Paketti:Step Mute APCKey25 Step 01`…`40 [Trigger]` (40-pad grid), and `Paketti:Step Mute Launchpad Step 01`…`64 [Trigger]` (64-pad grid).

### 2026-07-21 - Feature: Toggle 0G01 at Current Row
Added a one-shot toggle that writes `0G01` into the first effect column of the current pattern's current row — or wipes it if that row already holds `0G01`. Press once to place `0G01`, press again on the same row to clear it. Complements the existing full-pattern and from-cursor `0G01+0D00`/`0G01+0U00` fills. Available as menu entry `Pattern Editor:Paketti:Effect Columns:Toggle 0G01 at Current Row`, keybinding `Pattern Editor:Paketti:Toggle 0G01 at Current Row`, and MIDI mapping `Paketti:Toggle 0G01 at Current Row [Trigger]`.

### 2026-07-18 - Feature: Microtunings — full Byzantine chant octoechos (theory + performance practice)
Expanded the true Byzantine chant tunings in the Paketti Microtonal Tunings dialog/menu from four genera to the **complete eight-Tone octoechos**, grounded in Panteli & Purwins (2013, Journal of New Music Research 42(3), Table 1) — the authoritative study of the 72-moria Chrysanthine system. Twelve presets in total. **Theoretical (6):** First / First Plagal (Diatonic 10-8-12), Second (Soft Chromatic 8-14-8), Second Plagal (Hard Chromatic 6-20-4), Third / Grave (Enharmonic 12-12-6), Fourth (the irregular Tone, 8-12-12), and Fourth Plagal (Diatonic 12-10-8). The four genera shipped earlier today are now correctly named for the Tone each one is (Second, Second Plagal, Third/Grave, Fourth Plagal). **Performance practice (6):** the same six Tones rebuilt from the empirically measured step sizes in the same study (94 chants, four master cantors) — small steps run wider and the large ~333-cent step is compressed to ~277 cents, giving an audibly "as actually sung" set; each is octave-normalized so it stays octave-repeating. Apply any of them to the selected instrument from the Microtonal Tunings dialog. Menus/keybindings unchanged.

### 2026-07-18 - Feature: Double Harmonic (Byzantine) scales across Arpeggiator, Phrase Generator & Microtunings
Extended the Double Harmonic (Byzantine) scale family — added for the Music Mouse earlier today — into three more places. **Arpeggiator**: four new modes in the Mode dropdown — **double harmonic**, **hungarian minor**, **phrygian dominant** and **harmonic major** — with correct per-degree triads (two exotic triad qualities, `majb5` = major-third-plus-diminished-fifth and `sus2b5`, were added so the augmented-second degrees of Double Harmonic and Hungarian Minor build correctly instead of crashing). **Phrase Generator**: four new scales in the scale popup — **Double Harmonic / Byzantine (C)**, **Hungarian Minor (C)**, **Phrygian Dominant (C)** and **Harmonic Major (C)** (all correctly spelled; note the pre-existing "Spanish/Phrygian Dominant" entry is actually a Double Harmonic scale — left as-is to avoid changing existing output). **Microtunings** (Paketti Microtonal Tunings dialog / menu): eight new tuning presets — the four Double Harmonic family scales as 12-TET scale-as-tuning, plus four *true* Byzantine chant genera in the authentic 72-moria (72-EDO) system: **Byzantine Diatonic** (moria 12-10-8), **Soft Chromatic** (8-14-8), **Hard Chromatic** (6-20-4) and **Enharmonic** (12-12-6). Menus/keybindings for the three tools are unchanged.
...
### 2026-07-06 - Fix: PCM Writer — Create A&B truly no longer opens the Parameter Editor; Harmonic Drawbars on both views; both waves refresh on instrument switch; per-wave harmonic memory
Four fixes to the PCM (Single Cycle) Waveform Writer's Wavetable A&B workflow. (1) The real reason "Create A&B" was still popping the Parameter Editor: the device loader itself opens it. When your Paketti "device load behaviour" preference is set to open the Parameter Editor, every native device loaded (the *Instr. Macros + Wavetable Mod *LFO that A&B creation spawns) opened it directly — bypassing the earlier auto-open suppression. Fixed by loading those internal A&B devices silently, so the editor no longer opens on Create A&B / Random AKWF A&B. (2) The on-canvas Harmonic Drawbars are now shown on the panel in BOTH the Full view and the Canvas Only view (previously Canvas Only only). (3) When you switch instruments while the dialog is open, BOTH the Wave A and Wave B canvas displays now reload from the newly selected instrument (slot 1 → Wave A, slot 2 → Wave B) — previously, once Live Pickup was active, switching instruments left the displayed waves stale. (4) Each of Wave A / Wave B now remembers its own harmonic drawbar settings in-session: editing harmonics on Wave A, switching to Wave B, and switching back to A restores A's drawbars (previously they were reset on every switch). Keybinding to open the dialog is unchanged: `Global:Paketti:Show Paketti Single Cycle Waveform Writer...`.

### 2026-07-06 - Improvement: PCM Writer — no auto-open on Create A&B, Harmonic Drawbars in Canvas Only view
Three refinements to the PCM (Single Cycle) Waveform Writer's Wavetable A&B workflow. (1) Using "Create A&B" (or "Random AKWF A&B") no longer pops the Parameter Editor open. Creating the wavetable spawns a "Wavetable Mod *LFO" device, and if you had the Parameter Editor's auto-open enabled that device would trigger it — now A&B creation briefly suppresses auto-open so it stays out of the way. This does NOT change your auto-open preference; it only mutes it for the moment of creation, so auto-open still works everywhere else. (2) The Harmonic Drawbars are now available in the Canvas Only view: the on-canvas Wavetable A&B panel gained an 11-bar harmonic drawbar section along its bottom — left-drag a bar to add that harmonic on top of your current wave, right-click to zero it (it augments Wave A/B directly, auto-entering harmonic mode on first touch). (3) To make room for the drawbars, the "Wavetable A&B" title font is smaller and the Amplitude/Offset/Speed bars are tighter in the Canvas Only view. Keybinding to open the dialog is unchanged: `Global:Paketti:Show Paketti Single Cycle Waveform Writer...`.

### 2026-07-06 - Fix: PCM Writer — updates live when you switch tracks/instruments
With the PCM (Single Cycle) Waveform Writer dialog open, switching from a track/instrument that has no Wavetable A&B (12st_WT) setup to one that does now updates the dialog immediately — the LFO Amplitude/Offset/Speed controls and the on-canvas Wavetable A&B panel refresh on the spot, instead of only after closing and reopening the dialog. Previously the dialog only watched for sample changes, so a plain track/instrument switch left everything stale. It now also watches the selected-track and selected-instrument, refreshing the same way a reopen does (showing/populating the LFO controls and auto-entering Live Pickup for a freshly-selected wavetable), and hiding those controls again when you switch to a track without a wavetable. The new observers are detached on dialog close and before a New/Load Song, preserving the song-lifecycle crash safety. Menu/keybinding to open the dialog is unchanged: `Global:Paketti:Show Paketti Single Cycle Waveform Writer...`.

### 2026-07-06 - Fix: PCM Writer — Harmonic Drawbars now AUGMENT the waveform instead of overwriting it
The Harmonic Drawbars in the PCM (Single Cycle) Waveform Writer are meant to *augment* your current waveform with added harmonics — but they were overwriting it: entering the mode replaced your wave with a sine (it forced the first drawbar to full), and each drag re-read the already-modified wave as its starting point, so harmonics compounded and your original waveform was lost. Fixed by capturing the waveform as a fixed "fundamental" the moment you enter Harmonic Drawbar mode and always adding the drawbar harmonics ON TOP of that fixed fundamental. Now: entering the mode leaves your waveform untouched (all drawbars start at zero); dragging a drawbar adds that harmonic on top of your wave; and pulling a drawbar back to zero cleanly removes it, returning to your original waveform. When editing a Wavetable A&B (12st_WT) instrument the fundamental is re-based correctly each time you switch between Wave A and Wave B, and reopening a harmonic-encoded instrument reconstructs the fundamental so the drawbars keep editing rather than doubling. Menu/keybinding to open the dialog is unchanged: `Global:Paketti:Show Paketti Single Cycle Waveform Writer...`.

### 2026-07-06 - Feature: PCM Writer — two-scene view (Full / Canvas Only) + on-canvas Wavetable A&B panel
The Paketti PCM (Single Cycle) Waveform Writer dialog now has two views you switch with a **"View" switch above the canvas**: **Full** (all the button rows plus the canvas, exactly as before) and **Canvas Only** (a minimal view showing just the canvas, so you can draw and shape without the surrounding toolbars). A **"Boot into Canvas Only view"** checkbox next to the switch lets you make the dialog open straight into the minimal view every time (saved in preferences); off by default, so nothing changes unless you opt in. Switching views rebuilds the dialog instantly. To the right of the waveform there is now an **on-canvas Wavetable A&B panel** that fills the previously-empty space: when the instrument is NOT yet a Wavetable A&B (12st_WT) setup it shows two clickable buttons — **Create A&B** (builds the wavetable from your current Wave A + Wave B) and **Random AKWF A&B** (builds it from two random AKWF waveforms); once a Wavetable A&B setup is detected the panel turns into three live, drag-to-edit bars — **Amplitude**, **Offset** and **Speed** — that drive the Wavetable Mod LFO directly on the canvas, so you no longer have to jump out to a separate parameter editor to morph between Wave A and Wave B. Keybinding to open the dialog is unchanged: `Global:Paketti:Show Paketti Single Cycle Waveform Writer...`.

### 2026-07-06 - Improvement: PCM Writer — press "1" to toggle Wave A/B, red/blue Edit buttons
In the Paketti PCM (Single Cycle) Waveform Writer dialog you can now press the **1** key to instantly toggle between editing Wave A and Wave B on the canvas — no need to click the "Edit A" / "Edit B" buttons anymore (they still work, and clicking still switches). The "1" key preserves your cursor position and updates the canvas display immediately. The Edit buttons are now colored to match the canvas wave theme: **Edit A is red** and **Edit B is blue** when active (gray when inactive), matching the red Wave A and blue Wave B lines drawn on the canvas. The "1" toggle is ignored while you are typing into the hex value field.

### 2026-07-04 - Feature: Parameter Editor — per-plugin reorder / hide / rename (Customized Ordering Mode)
The Parameter Editor can now be customized per plugin. With "Customized Ordering Mode" enabled (Paketti Preferences → Parameter Editor), a new "Configure..." button (also `Mixer:Paketti Gadgets:Configure Parameter Editor for Selected Device` and keybinding `Global:Paketti:Configure Parameter Editor for Selected Device`) opens a dialog where you can, for the selected device: reorder parameters (↑/↓), hide parameters (Show checkbox), and rename their displayed labels — with Save / Reset to Plugin Default / Cancel. This is a display layer only: it never changes the plugin's real parameter order or names, so drawing, automation, expose-on-mixer, and A/B all still act on the real parameters. The config is keyed by the device's device_path and persists in Paketti preferences. When the mode is off, or a device has no config, the editor behaves exactly as before (the Wavetable Mod *LFO skip-first-3 rule is preserved). Verified via the real BuildDisplayedParameters path: mode-off equals baseline, reset restores baseline, and reorder/hide/rename apply as configured. Menu: `Mixer:Paketti Gadgets:Selected Device Parameter Editor...`.

### 2026-07-04 - Improvement: Parameter Editor — structured per-plugin config schema groundwork
Added the structured Renoise document models and preference-list storage for future per-plugin Parameter Editor customization configs, keyed for plugin/device-specific display-layer data. This is groundwork only: it stores no active behavior yet, but lays down the non-stringly, preferences-backed schema needed for future reorder/hide/rename configuration without touching Renoise's real parameter order. Paketti Preferences: `Main Menu:Options:Paketti Preferences...`, `Main Menu:Tools:Paketti:!Preferences:Paketti Preferences...`.
...
Three Parameter Editor improvements. (1) "Expose on Mixer" now works with Automation Sync OFF — dragging a parameter bar surfaces it in the mixer whenever the option is ticked (it was previously gated behind Automation Sync, so it appeared to do nothing). (2) New "Expose Automated Parameters on Mixer" command surfaces only the selected track's parameters that actually have automation (`is_automated`), not every parameter — keybinding `Global:Paketti:Expose Automated Parameters on Mixer`, MIDI `Paketti:Expose Automated Parameters on Mixer [Trigger]`, menu `Mixer:Paketti Gadgets:Expose Automated Parameters on Mixer`. (3) New "Grid stripes" checkbox paints alternating light/dark backgrounds behind each parameter column for grid-style reading. Two further requests (per-plugin parameter reorder/hide/rename, and a Renoise↔Ableton Live sample round-trip) are captured as feasibility studies in `Research/parameter-editor/feasibility.md` with proposed designs. Menu: `Mixer:Paketti Gadgets:Selected Device Parameter Editor...`.
...
### 2026-07-03 - Research/Debug: Squarp Hapax feasibility probe for Groovebox 8120
Added a Hapax MIDI probe (modeled on the APC probe) to study whether the Squarp Hapax can get the same 8120 controller integration as the APC Key 25 / LPD8 / MidiMix. Feasibility finding (from the 159-page manual): the Hapax can send pad notes and assignable encoder CCs to the computer, but has NO documented way to light its own pads/steps from incoming MIDI (no SysEx anywhere in the manual) — so the defining APC-style feature, the 8120 painting step state back onto the controller's LEDs, is not supported; only a one-way "Hapax plays the 8120" path is feasible. The probe lets Josh confirm this empirically on hardware — and it opens a self-contained **copyable window** so no one has to enable the hidden scripting terminal. "Hapax Probe — Open Window" shows a dialog with a live, copyable log (multiline text field — click in, select-all, copy, paste it back) plus three numbered buttons: 1) Open probe (logs the device inventory + every incoming pad/encoder message), 2) Test grid LEDs (blasts Note On + CC sweeps so you watch whether anything lights), 3) Close probe. The whole test runs from that one window. Menu: `Main Menu:Tools:Paketti:!Preferences:Debug:MidiControllers:Hapax Probe — Open Window/Test grid LEDs/Close`. Keybindings: `Global:Paketti:Paketti Groovebox 8120 Hapax Probe Open`, `... Close`, `... Test LEDs`, `... Hapax Probe Window`. Full write-up in `Research/hapax-8120/FEASIBILITY.md`.
...
### 2026-07-03 - Feature: Batch Convert XRNI Folder to hardware formats (8SVX, 16SV, IFF, Octatrack, ITI, Digitakt Chain)
Extends the "drop a folder of `.xrni`, get files back beside them" idea from PTI to five more targets. Point any of these at a folder of `.xrni` instruments (subfolders walked recursively, output written next to each source so structure is preserved, temp instruments deleted after each so the 255-instrument cap is never hit): **8SVX** (Amiga 8-bit) and **16SV** (Amiga 16-bit) and **IFF** — one file per sample, single-sample instruments write `name.ext`, multi-sample write `name-01.ext`, `name-02.ext`…; **Octatrack** — writes `name.wav` + `name.ot` (slice metadata) per instrument; **ITI** — Impulse Tracker *instrument* files (note: `.iti`, not `.it` modules), verified by an export→re-import roundtrip; **Digitakt Chain** — combines the instrument's samples into one chained `.wav` with CUE slice markers. Each reuses the existing single-file exporter (given an optional output-path so batch skips its Save dialog) — no exporter logic duplicated. Menus: `Main Menu:File:Paketti Export:Batch Convert XRNI Folder to <FORMAT>...` and `Disk Browser:Paketti:Import/Export:Batch Convert XRNI Folder to <FORMAT>...` for each of the six format names (`8SVX`, `16SV`, `IFF`, `Octatrack (WAV+.ot)`, `ITI`, `Digitakt Chain`). Keybindings: `Global:Paketti:Batch Convert XRNI Folder to <FORMAT>`. MIDI Mappings: `Paketti:Batch Convert XRNI Folder to <FORMAT>`.

### 2026-07-03 - Feature: Batch Convert XRNI Folder to PTI (+ PTI export LFO crash fix)
Drop a whole folder of `.xrni` instruments — organised into subfolders by sound type if you like — and Paketti converts every one to a `.pti`, writing each file right next to its source `.xrni` so your folder structure is preserved for free. It walks subfolders recursively, skips nothing, and removes each instrument after export so you never hit Renoise's 255-instrument limit no matter how many hundreds of files you convert. This closes the gap where PTI was the one format with no batch export — previously you had to export instruments one at a time. **Also fixed a crash in all PTI export**: any instrument carrying an LFO modulation device aborted the export because the code read a non-existent `.amount` property on the LFO — corrected to Renoise's actual `.amplitude` (verified live against `SampleLfoModulationDevice`), so `pti_savesample`, "Convert REX/RX2/ITI to PTI", and the new batch converter now all survive LFO-bearing instruments. Menu: `Main Menu:File:Paketti Export:Polyend (PTI) Batch Convert XRNI Folder to PTI...`, `Instrument Box:Paketti:Instruments:Batch Convert XRNI Folder to PTI...`, `Disk Browser:Paketti:Import/Export:Batch Convert XRNI Folder to PTI...`, and a button in the **Polyend Buddy** dialog's Convert row (`XRNI Folder→PTI (Batch)`). Keybinding: `Global:Paketti:Batch Convert XRNI Folder to PTI`. MIDI Mapping: `Paketti:Batch Convert XRNI Folder to PTI`.

### 2026-07-02 - Improvement: Music Mouse — big feedback pass (no-flood patterns, tight chords, arps, layout)
A wide round of Music Mouse improvements. **Sound**: changing any dropdown/checkbox/switch (harmony, voices, format, transposition, etc.) no longer re-strikes the chord — the new state is heard on the next mouse move or i/o/p punch, and nothing ever triggers while frozen. Chord changes are now **batched into single note-off/note-on calls**, killing the per-voice MIDI jitter/flam; unchanged voices are left ringing. **Patterns**: with *Pattern Applies = Melody/Bass* the contour now sequences **only that voice over a sustained chord** instead of re-triggering the whole chord every step (the flood bug). Contour length raised from 16 to **64** with an 8/16/32/64 switch and Len+ feedback at the cap. **Arpeggiate** gains sub-modes **Up / Down / Scatter / Strum** (Strum writes the chord across note columns with rising delay-column offsets). **Gravity Play** gains a divisor — hit a seed every beat / 4th / 8th / 16th. **space** is captured before anything else so it never bleeds into the pattern editor; recording auto-widens the track to the voice count. **Sound keys**: i / o / p punch three **saved favorite waveforms** (set + persisted in the panel), å re-triggers the current sound, shift-i round-robins the favorites. **Tuning** is now a dropdown (tab still cycles); **< / >** transpose alongside z / x. **Layout**: strong proportional labels (no wide mono), tighter mute row, aligned Waveform+Mode+Create row, Record→Pattern is a **RED-when-armed button**, help text moved to tooltips, plus **Hide pianos** and **Hide details** toggles. The **Keyboard Map** is rebuilt as clickable, MIDI-mappable buttons (cmd-M highlights them) with de-duped title, bold headings and even alignment. Keybindings/MIDI: `Global:Paketti:Music Mouse ...`; new per-key MIDI mappings `Paketti:Music Mouse Key ...`.
...
### 2026-06-26 - Feature: "Dialog of Dialogs — Screenshot All" (batch-capture every Paketti dialog)
A new `PakettiScreenshotAllDialogs(dir)` walks every entry in the Dialog of Dialogs, opens each, screenshots it (macOS `screencapture`, fired detached so it never blocks Renoise's main thread), and closes it — saving `NNN_<dialog name>.png` into a folder. Runs in a ProcessSlicer so the UI renders between open and capture. Menu: `Main Menu:Tools:Paketti:Xperimental/WIP:Dialog of Dialogs Screenshot All`. Drivable headlessly via PakettiMCP (open a dialog + the `paketti_screenshot` tool + read the PNG), which is what lets an assistant/bot attach a dialog screenshot to a chat. First run captured 40 dialogs cleanly (verified — e.g. the full Paketti Preferences panel) before freezing on "Paketti Action Selector", whose open() blocks on a native modal; such dialogs are now added to the smoke-test SKIP_LIST so subsequent runs skip them.

### 2026-06-25 - Improvement: PakettiMCP — capability introspection tool (paketti_introspect)
`paketti_introspect` lists Paketti global functions matching a filter, each with its parameter count, vararg flag, and source file:line (via `debug.getinfo`) — so a client can discover what's callable (and with how many args) before driving it via `paketti_eval`. Added live via the in-place hot-reload (91 → 92 tools, no restart) and verified: a `PakettiPolyendExport` filter returned all four export functions with correct arities. Dev-tool only.

### 2026-06-25 - Improvement: PakettiMCP — undo-checkpoint + render-to-file dev tools (Tier 2 complete)
Two more PakettiMCP dev tools, both added live via the new in-place hot-reload (no Renoise restart): `paketti_undo_checkpoint` names the current undo step (`song:describe_undo`) so a test mutation can be reverted with a single Undo, and `paketti_render` renders the song to a WAV (Renoise's GUI blocks during the render, so the file on disk — not MCP — is polled for completion). Both verified live: render produced a 1.2 MB stereo 16-bit/44.1 kHz WAV in ~1 s, and the hot-reload took the tool count 89 → 91 without dropping the MCP connection. Dev-tool only; no user-facing changes.

### 2026-06-25 - Improvement: PakettiMCP — non-wedging reload, socket watchdog self-heal, structured eval, screenshot + file tools
Hardened the PakettiMCP dev server. **Reload no longer wedges the socket:** `PakettiMCPReloadTools` (and a new `paketti_reload` tool) now reload only the tool files in place via `router.load_tools_dir` — they no longer re-`require` the server module, which used to orphan the bound socket so port 19714 stayed `LISTEN` but answered nothing. **Watchdog self-heal:** the 100 ms poll now retries `create_server` (throttled to ~1 s) instead of giving up after a socket error, and reaps half-open clients after ~30 s. **New tools:** `paketti_eval_json` (structured `{ok,type,value,stdout,error}` with captured `print`), `paketti_screenshot` (grab the Renoise front window/dialog to a PNG — fires `screencapture` detached so it can never block Renoise's main thread), and `paketti_read_file` / `paketti_write_file`. All verified live (reload kept MCP responsive; screenshot captured a Paketti dialog without freezing). Dev-tool only; no user-facing menu/keybinding/MIDI changes.

### 2026-06-25 - Feature: Polyend project export splits long patterns and chains them in the playlist
A Renoise pattern longer than Polyend's 128-step max is now split into multiple `.mtp` parts during full-project export, and the device playlist is expanded so the parts play back in order — a 200-line pattern becomes `pattern_01.mtp` (128 steps) + `pattern_02.mtp` (72 steps), and a Renoise sequence of [Long-A, Short-B, Long-A] exports to playlist `[1, 2, 3, 1, 2]` automatically. Patterns are numbered sequentially in first-appearance order (matching how the device names them), and split parts are labelled `Name (1/2)`, `Name (2/2)` in `patternsMetadata`. Verified live via PakettiMCP: 3 files with the correct 128/72/64-step lengths and the exact `[1,2,3,1,2]` playlist. No menu/keybinding/MIDI changes (rides on the existing Export Polyend Project).

### 2026-06-25 - Feature: Polyend project export now writes instruments (.pti) — exported songs have sound on the device
Exporting a Renoise song as a Polyend project previously wrote only the patterns, `project.mt` and metadata — no instruments, so the project loaded on the Tracker with empty instrument slots. The project exporter now also writes each Renoise instrument that has sample data to `instruments/<N> <name>.pti` (N = 1-based instrument number, matching real device files), using Paketti's existing PTI exporter — so an exported project actually plays. The pattern step instrument byte was also corrected to a direct device mapping (Polyend 0-based index = Renoise `instrument_value`, clamped to 0-47), replacing a fragile `-2` offset that assumed one specific import slot scheme; this lines the step bytes up with the exported `.pti` filenames. The whole-project exporter is also now available headless (`PakettiExportPolyendProjectToFolder`). Menu/MIDI unchanged (existing `Export Polyend Project`). Instrument step-byte mapping verified offline; the `.pti` write path uses the same exporter already validated for single-instrument export.

### 2026-06-25 - Feature: Polyend MTP export — long-pattern splitting, selection/clipboard export, MIDI mappings
Three additions to Renoise→Polyend pattern export. **Long patterns auto-split:** a Renoise pattern longer than Polyend's 128-step maximum now exports as multiple `.mtp` files (a 200-line pattern → `pattern_part01.mtp` lines 1–128 + `pattern_part02.mtp` lines 129–200) instead of being truncated — verified live, every note landing at the correct step. **Selection / clipboard export:** the selected line-range in the Pattern Editor exports to a single `.mtp` (lines 50–80 → a 31-step pattern), so you can lift a chunk of a Renoise song straight onto the Tracker. Both ride on a new line-range parameter on the exporter (`start_line`/`num_lines`), keeping the byte-faithful container. Menu: `Main Menu:Tools:Paketti:Xperimental/WIP:Polyend:Export Selection to MTP` and `...:Export Pattern to MTP (auto-split >128)`. MIDI Mappings (drive export from a controller): `Paketti:Export Pattern to Polyend MTP`, `Paketti:Export Selection to Polyend MTP`, `Paketti:Export Pattern to Polyend MTP (auto-split)`, `Paketti:Export Polyend Project`, `Paketti:Import Polyend MT Project File`. All exports validated against the official Polyend `tracker-lib` and the reference parser.

### 2026-06-25 - Fix: Polyend project (project.mt) export — device-faithful, correct BPM, reloadable filenames
Exporting a Renoise song as a Polyend project wrote a malformed `project.mt`: it hand-rolled a 2096-byte file from zeros (real device projects are 2320 bytes), wrote the tempo at offset `0x80` (the device reads `0x1C0`, so exported projects loaded at 0 BPM), used a `fileStructureVersion` of `1.0.0.0` that desynced the version-gated project-name offset, and named the pattern files `pattern_000.mtp` (0-based) while the device and Paketti's own loader use `pattern_01.mtp` (1-based) — so exported projects couldn't be reloaded. The exporter now starts from an embedded **real-device `project.mt` template** (a clean 2320-byte project the Tracker itself wrote, with valid firmware/structure version and instrument/delay/reverb config) and patches the playlist, tempo (float32 @ `0x1C0`, from the Renoise BPM), project name (@ `0x80C`), and track names onto it — so the output is byte-shaped exactly like a device file. Pattern files are now written `pattern_NN.mtp` 1-based to match the playlist and reload correctly. Verified live via PakettiMCP (real Renoise → 2320-byte project.mt, tempo read back at the right offset) and offline (export → re-read with Paketti's corpus-validated reader: BPM, name, playlist all round-trip). No menu/keybinding/MIDI changes.

### 2026-06-25 - Fix: Polyend pattern (.mtp) export was malformed — now byte-identical to real device files
Exporting a Renoise pattern to Polyend `.mtp` produced a file the Tracker rejected. The header wrote `"--"` as the file id (real Tracker files use **`"KS"`**), the per-track "last step" marker was written only on track 1 and zeroed on tracks 2–16 (the device writes it on **all** tracks), and the CRC handling was broken — it never wrote the 4 CRC bytes, then `seek(end,-4)` overwrote the last step's note data, leaving a file **4 bytes too short** (12332 instead of 12336) so track-count detection failed on import. The writer now emits a byte-faithful container: `"KS"` id, type 2, firmware 1.9.1.1, structure 5.5.5.5, the size field = total file size, the last-step marker on every track, and a zero CRC (real device files and the official Polyend `tracker-lib` both store 0). The exported header is now **byte-identical** to a real device pattern's header, the file is exactly 12336 bytes for 16 tracks, and — verified against the official Polyend `tracker-lib` (the same code the Polyend web editors use) — a Paketti-exported `.mtp` parses identically to a real device file. Verified with an offline harness: export → re-read with Paketti's own reader (notes, note-offs, FX preserved) → parse with the official library. No menu/keybinding/MIDI changes.

### 2026-06-25 - Fix: Polyend project import read BPM from the wrong offset (always got 0 / fell back to default)
Importing a Polyend Tracker project (`project.mt`) into Renoise read the song tempo from byte offset `0x80`, which is `0.0` on every real device file — so the imported BPM silently fell back to a default and your project always came in at the wrong tempo. The correct location is a little-endian float32 at **`0x1C0`**. This was verified three ways: by decoding Polyend's own official 2324-byte project template (130.0 at `0x1C0`), against the official `tracker-lib` source, and against real device projects whose tempo is in their folder name — "Treasure Island **140bpm**", "Bios Creatures **170bpm**", and a 130 BPM project — all of which now import at exactly 140 / 170 / 130. Also made the project-name reader firmware-version-aware (newest Tracker+/Mini firmware stores the name at `0x810`, older 16.x files at `0x80C`, legacy/OG at `0x600`) so project names import correctly across all firmware. Verified with an offline harness running Paketti's actual reader against real `.mtp`/`project.mt` files. No menu/keybinding/MIDI changes.

### 2026-06-24 - Fix: REX/RX2/ITI→PTI conversion crashed on instruments with a Stepper (or LFO) modulation device
Converting a sample to **.PTI** (the Polyend Buddy `REX/RX2/ITI→PTI` button, and any PTI export) crashed with `unknown property or function 'attack' for an object of type 'SampleStepperModulationDevice'` whenever the instrument's modulation set contained anything other than a plain AHDSR — most commonly a **Pitch Stepper**, which Paketti's default instrument adds, so sliced REX2 kits hit it immediately. The PTI modulation extractor (`extractModulationForPTIExport` in `PakettiPTILoader.lua`) detected the device type by *reading* a type-specific property (`device.attack`, `device.mode`), but Renoise **raises an error** when you read a property that doesn't exist on that device type — so probing `.attack` on a Stepper aborted the whole conversion. It now identifies each device by its fixed `device.name` (`"… AHDSR"` / `"… LFO"`) and only reads properties that type actually has; Stepper / Envelope / Fader / Key Tracking / Operand / Velocity Tracking devices (which have no PTI envelope/LFO equivalent) are skipped cleanly. Two latent crashes in the same function were fixed in passing: AHDSR's decay stage is read from `duration` (Renoise has no `.decay`), and the LFO amount is read from `amount` (Renoise has no `.amplitude`). Verified with an offline harness that replicates Renoise's strict property access — reproducing the original crash and confirming Stepper/AHDSR/LFO/mixed instruments all convert without error. No menu/keybinding/MIDI changes.
...
### 2026-06-16 - Feature: Music Mouse — Laurie Spiegel's "Intelligent Instrument" (1986), reborn in Renoise
A from-scratch Canvas port of Laurie Spiegel's **Music Mouse** (40th-anniversary tribute). Move the mouse over a four-sided piano-keyboard grid to play voices — mouse X drives the treble chord, mouse Y the bass melody — with the position quantized to a chosen harmony so you can't play wrong notes. **Harmony modes** `q w e r t y` (Chromatic / Octatonic / Middle-Eastern / Diatonic / Pentatonic / Quartal, from Spiegel's own scale + voice-stacking tables); **transposition** `z x c`; **melodic patterning** `a` + `0-9` (ten contour arrays, with a built-in step-bar Pattern Editor and an "applies to" target — all voices / melody / bass); **voicing** `d f`, **articulation/treatment** cyclers `g`/`h`; **articulation** `/`, **loudness** `, .`, **muting** `shift-1..9`; **dual tempo** `- + [ ] \` with **Sync-to-BPM** (the pattern player locks to the song BPM/LPB, live). Sound is the **selected instrument** or a one-click **Pakettified** waveform instrument (loads the Paketti Default Instrument, renders a single-cycle wave into it, tuned via the PCM Writer convention so it's editable there) — **Bell** mode (non-looping baked decay) is the default, **Sustain** (looping) the alternative; waveforms Triangle/Square/Saw/Sine/Rounded-Square/Pulse/Diode/Gauss (`u i o p`, `m` cycles). **4–9 voices** (4 = classic Music Mouse; 5–9 stack extra thirds into rich 7th/9th/11th/13th chords). Beyond the original: **Keyjazz punch** (`j` — aim silently, fire with `u/i/o/p`), **freeze** (`space`), **lock/retain** (`enter` / `shift-enter`), **Light/Dark** theme (`k`), an in-app **Keys...** reference, and **Record → Pattern** (right-shift — switches to the Pattern Editor, turns on Edit Mode + Follow, starts playback, and imprints what you play onto the selected track at the playhead, write-on-trigger, with your loudness as the volume column). Plus **gravitation seeds** — left-click drops a marker chord — and **Gravity Play** (`;` / button / MIDI) sequences the seeds in recorded order at tempo. Tempo, loudness and the seeds persist across close/reopen and reloads.
- Menu: `Main Menu:Tools:Paketti:Music Mouse...` (also `Main Menu:Tools:Paketti:Instruments:Music Mouse...` and `Instrument Box:Paketti:Music Mouse...`)
- Keybinding: `Global:Paketti:Music Mouse Show/Hide`
- MIDI Mapping: `Paketti:Music Mouse Show/Hide`, `Paketti:Music Mouse BPM (Slider)`, `Paketti:Music Mouse Tempo (Slider)`, `Paketti:Music Mouse Gravity Play (Toggle)`

### 2026-06-16 - Fix: HyperEdit Sculpt/Gang — holding now keeps ramping, and the Gang interval self-activates
Two fixes to the HyperEdit Sculpt/Gang tools. **Holding "HOLD SCULPT" while stopped now keeps moving the parameter** for as long as you hold it (REL ramps, Random keeps scattering) instead of applying just once — previously you had to release and re-press to make each change. (While playing, the playhead still drives once-per-pass accumulation as before.) **Changing "Gang every" or "offset" now auto-enables the Gang checkbox**, so dialing the interval just works instead of silently doing nothing when the checkbox happened to be off.

### 2026-06-16 - Feature: Microtonal Tunings — load Scala (.scl) files from the tunings/ folder, with eight Wendy Carlos scales bundled
The Microtonal Tunings system now reads standard **Scala (`.scl`) files** straight from Paketti's `tunings/` folder and turns each into a ready-to-apply tuning preset. Drop any `.scl` file into `tunings/` and it appears automatically in the Microtonal Tunings dialog popup (and its A/B comparison) and gets its own Apply menu entry the next time the tool loads — no scripting required. The parser handles both cents and ratio (`3/2`, `17/16`, bare-integer) pitch lines, and supports non-octave scales (period taken from the last pitch), applied via Renoise's native Scala tuning. Eight Wendy Carlos scales ship bundled: the famous **non-octave-repeating** Alpha (78c step, fifth/9, 18 notes), Alpha Prime (39c, fifth/18, 36 notes), Alpha Benson-optimized (18 notes), Beta (63.8c, fifth/11, 22 notes), Beta Prime (31.9c, fifth/22, 44 notes) and Gamma (35.1c, fifth/20, 35 notes) — whose period of repetition is the perfect fifth, not the octave — plus the octave-repeating just-intonation Carlos Harmonic and Carlos Super Just (12 notes each). These complement the existing compact algorithmic Carlos generators rather than replacing them. Use the existing **Reset to 12-TET** to return to standard tuning.
- Menu: `Main Menu:Tools:Paketti:Microtonal Tunings:Apply <scale>` — one entry per `.scl` file found in `tunings/` (also under `Instrument Box:Paketti:Microtonal Tunings`)
- Bundled files: `tunings/carlos_alpha.scl`, `carlos_alpha2.scl`, `carlos_alphab.scl`, `carlos_beta.scl`, `carlos_beta2.scl`, `carlos_gamma.scl`, `carlos_harm.scl`, `carlos_super.scl`

### 2026-06-14 - Feature: HyperEdit — Gang (shape every Nth step at once)
The Sculpt toolbar gains **Gang**, ported from the Cirklon "GANG by step/row" idea. Turn on the **Gang** checkbox and set **Gang every** N (with an **offset**): the matching steps — every step when N=1 (the whole row), every 3rd step when N=3, etc. — are **highlighted** on every armed row's canvas. While Gang is on, **drawing on any one highlighted step moves every ganged step of every armed ("S") row together** — the "S" toggles pick which rows participate, so one drawn step writes across all armed rows at once (drawing a non-highlighted step stays a normal single step). And engaging the Sculpt hold shapes that **whole ganged set at once, live** — move the value (the A/B boxes, or Alt+drag a canvas for a live knob A) and every ganged step of every armed row moves together, using the active mode (ABS overwrites, REL offsets from each step's value at the moment you engaged, Random scatters within A…B). Scope is each row's own step cycle. Gang replaces the single-step playhead behaviour while it's on; turn Gang off to go back to shaping one playing/edit step at a time. Gang on/off, interval and offset persist in preferences.
- Menu: (in-dialog — **Gang** checkbox, **Gang every** and **offset** value boxes in the Sculpt toolbar)

### 2026-06-13 - Feature: HyperEdit — Cirklon-style Sculpt / Random modes
HyperEdit gains a **Sculpt** toolbar (above the rows) that ports the four sculpt modes from the Cirklon sequencer. Pick a mode — **Regular** (off), **Sculpt ABS**, **Sculpt REL**, **Random ABS**, **Random REL** — set the value(s) (−127…127, where 127 is thought of as 1.0), then **engage the hold** while the pattern plays. The toolbar shows the right controls for the mode: ABS and REL show a **single** value box; the Random modes show **two** boxes as a range (`-12 <> +23`); Regular shows none to shape the currently-playing step of every armed row, once each pass. **Sculpt ABS** overwrites the playing step with knob A. **Sculpt REL** offsets it by knob A every pass, cumulatively (e.g. −3 walks each step down to 0, +3 walks it up to full). **Random ABS** replaces the playing step with a random value in the A…B range. **Random REL** adds a random A…B offset each pass, accumulating. Works in both Effect-parameter and Stepper modes. The hold can be driven four ways: the **HOLD SCULPT** button (momentary), **Alt+drag on a row's canvas** (momentary; mouse-Y becomes a live knob A — a plain drag still draws normally), a **keyboard shortcut** (toggle — Renoise has no key-up to hold against), or a **MIDI button** (momentary via note/CC release). Sculpt writes into the playing step whether or not it already had content, so ABS/REL/Random fill silent steps instead of leaving them quiet. Sculpt also works **while stopped**: engaging the hold acts immediately on the step under the edit cursor, and moving the edit cursor while held shapes each step you land on (playing still drives once-per-pass crossings, which is what REL needs to accumulate). Each row has an **S** arm toggle (green = armed); only armed rows respond, with **Arm All / Arm None** for convenience. Sculpt mode and knob A/B persist in preferences; the engaged state is always a live hold and never persists.
- Keybinding: `Global:Paketti:HyperEdit Toggle Sculpt Hold`
- MIDI Mapping: `Paketti:Paketti HyperEdit Sculpt Hold`
- Menu: (in-dialog — Sculpt mode popup, A/B value boxes, HOLD SCULPT button, per-row **S** arm toggles, Arm All / Arm None)

### 2026-06-12 - Feature: Groovebox 8120 — Per-Step Sample mode (Mode button)
The Groovebox 8120 dialog now has a **Mode** button (in the top control row, next to the 8/16/32 step switch) that flips all eight rows between two modes. **Single Sample mode** (MODE1) is the original behaviour: each row plays one sample chosen by its slider, and the control strip (`<` `>`, Clear, Random Steps, instrument dropdown, Load, RandomLoad, slider, Random, Show, …) is shown. **Per-Step Sample mode** (MODE2) shows, under every step checkbox, a 1–120 number field (type a value, or focus it and press Up/Down to nudge ±1; out-of-range snaps back) that chooses **which sample triggers on that step** — so each of the 16/32 steps can fire a different sample from the row's 120-sample kit. The first time you enter Per-Step mode the row instruments are remapped to one-note-per-sample (sample i → note i-1), so the note value selects the sample; from then on both modes write note numbers and the slider no longer choke-mutes. Existing songs are untouched until you opt in. Per-step sample numbers are session-only (in-memory) for now. Fetch reads per-step samples back from the pattern.
- Menu: (toggle via the in-dialog **Mode** button)
- Keybinding: `Global:Paketti:Paketti Groovebox 8120 Toggle Step Mode`
- MIDI Mapping: `Paketti:Paketti Groovebox 8120:Toggle Step Mode`
...
### 2026-06-11 - Improvement: Folded the `Paketti..` context-menu submenus into `Paketti` too (right-click menus)

Follow-up to the Tools-menu fold: the duplicate `Paketti..` submenu that also appeared in the right-click context menus is gone. Its entries now live under the single `Paketti` submenu in each context: Sample Editor (Manual Slicer, Oldschool Slice Pitch, SlicePro, SliceSafely, Slices to Pattern/Phrase, Curved Slice Creator), Pattern Editor (Command Wheel, Effect Column Curve Fill, Oldschool Slice Pitch Workflow, Slices to Pattern/Phrase, Equal/Random Slice Distribution), Instrument Box (SlicePro, SliceSafely), Instrument Phrases (Phrase Recording), and Phrase Editor (Command Wheel). The inner `Slices..` subcategory dropped its dots to `Slices`. Separately, the `Paketti:Experimental` context-menu entry "Create MIDI Control from Text File" (in the DSP Device, Sample FX Mixer, and Mixer right-click menus) moved into `Paketti:Xperimental/WIP` to match the canonical experimental folder. The Menu Configuration dialog's stored paths were updated in lockstep. No keybindings or MIDI mappings changed — menu paths only.
...
- Menu: `Main Menu:Tools:Paketti:Groovebox:LPD8 Lights — Lightshow (all) / Blink / Stars (random twinkle) / Scroll / Snake 1·2·3·4·8·7·6·5 / Snake 5·1·2·6·7·3·4·8 / Snake 5·6·7·8·4·3·2·1 / Stop Animation`
- Keybinding: `Global:Paketti:Paketti Groovebox 8120 LPD8 Lightshow`
...
- Menu: `Main Menu:Options:Auto-Start AKAI APC Key 25` (checkable)
- Menu: `Main Menu:Tools:Paketti:Groovebox:Auto-Start AKAI APC Key 25` (checkable)
- Preference: `PakettiEightOneTwentyAPCAutoStart` (default OFF)
...
- Menu: `Main Menu:Tools:Paketti:Groovebox:MidiMix Bridge Toggle (start headless LEDs)`
- Keybinding: `Global:Paketti:Paketti Groovebox 8120 MidiMix Bridge Toggle`
- Keybinding: `Global:Paketti:Paketti Groovebox 8120 MidiMix Refresh LEDs`
- MIDI Mapping: `Paketti:Paketti Groovebox 8120:MidiMix Refresh LEDs [Trigger]`
...
- MIDI Mapping: `Paketti:Paketti HyperEdit Open/Close`

### 2026-06-09 - Feature: Groovebox 8120 "Select Row (Knob 01-08 / 08-01)" — one knob sweeps the selection across the 8 rows

Two MIDI mappings for one absolute knob/fader: the 0–127 range is split evenly into the 8 rows, so sweeping it walks the **whole selection** through the rows — each position selects that row's track *and* its instrument and focuses the row, so the `Selected Row Step01–16` buttons and the MidiMix LEDs follow. 16 values per row. `Select Row (Knob 01-08)` goes 0→127 = row 1→8; `Select Row (Knob 08-01)` is the reverse (0→127 = row 8→1) for a fader whose travel feels inverted.

- MIDI Mapping: `Paketti:Paketti Groovebox 8120:Select Row (Knob 01-08)`
- MIDI Mapping: `Paketti:Paketti Groovebox 8120:Select Row (Knob 08-01)`
...
- Keybinding: `Pattern Editor:Paketti:ChordsPlus (Add 01)` through `ChordsPlus (Add 12)` (was `Chordsplus (Add NN)`)

### 2026-06-09 - Feature: HyperEdit Stepper mode — read and draw an instrument's Stepper modulation devices as looping step sequencers

HyperEdit can now edit **Stepper modulation devices** as well as track-device automation. A new **Mode** dropdown at the top of the HyperEdit toolbar switches between **Effect Parameters** (the original: each row edits a track device parameter and writes a pattern automation envelope) and **Steppers** (each row edits one Stepper modulation device on the *selected instrument*, writing directly into the Stepper's points). In Stepper mode the top "device" dropdown lists the instrument's sample modulation sets and the second dropdown lists the Stepper devices inside the chosen set (Volume / Cutoff / Resonance / Pitch / Panning / Drive Stepper). When you open it, HyperEdit assigns the selected sample's Steppers to the rows (then any remaining sets' Steppers) and reads each one in — the row's step count snaps to the Stepper's length, and the curve you see is the Stepper's actual points. Drawing on a row, the 0.0 / 0.5 / 1.0 set-all buttons, the step-count buttons, Clear All, and even live MIDI Write all write straight into the Stepper's points; the step-count buttons also resize the Stepper's loop length. Because a modulation Stepper loops over its length automatically while a note is held, this gives you a multi-row visual editor for several looping modulation sequences (e.g. volume wobble + filter sweep + pitch arp) on one instrument at once. Switching the selected instrument re-scans and re-assigns the rows. The whole drawing canvas, fill, and step-count machinery is shared with the original mode — only the read/write target changes. The mode is remembered in preferences (`PakettiHyperEditMode`). Crucially, a Stepper is **not** tied to pattern playback: it advances one step on every note-on of its instrument (trigger four notes and the stepper moves four steps). So in Stepper mode the playhead follower counts the instrument's note-on triggers as the pattern playhead crosses each line and advances the row highlights by that count (each row wrapping at its own Stepper length) — and live MIDI Write records into whichever step the Stepper is currently on, not the pattern line. The follower count resets when the transport (re)starts. Effect Params mode is unchanged.

- Menu: `Pattern Editor:Paketti Gadgets:Paketti HyperEdit (Steppers)`
- Menu: `Mixer:Paketti Gadgets:Paketti HyperEdit (Steppers)`
- Menu: `Instrument Box:Paketti Gadgets:Paketti HyperEdit (Steppers)`
- Keybinding: `Global:Paketti:Paketti HyperEdit (Steppers)`
- Preference: HyperEdit mode (`effect` / `stepper`), default `effect`

### 2026-06-09 - Fix: two always-on tools crash-proofed (SIGSEGV) on New Song / Load Song — Zero Crossings auto-snap and Trigger-on-Input

The New Song / Load Song crash kept recurring after the earlier batches because those targeted tools with dialogs. This one came from **always-on, no-dialog** features that watch the song in the background and were never told to let go when the song is swapped. **Zero Crossings auto-snap** (which runs all the time, regardless of whether its snap option is on) was watching the selected sample and the sample's selection, using a watcher it could never actually remove — so every New Song left it pointing at the destroyed song. It now uses a proper, removable watcher and detaches before the song is released. **Trigger Sample on Pattern Input During Record** had the same gap (latent, only when you enable it) and now detaches too. Both re-attach automatically to the new song. The codebase was also swept for any other always-on background watcher with this gap — none remain.

Two Groovebox 8120 workflow upgrades. **(1) Record button → Pakettified instrument:** pressing a row's Record button now loads a fresh Paketti Default Instrument chassis into that row's instrument slot *before* recording, so your recorded take lands inside a proper Paketti instrument (pitch-bend modulation + *Instr. Macros) instead of a bare one — and when you press Record again to stop, the recorded sample is mapped as the row's primary sample and pointed at the chassis's modulation set. Each Record press starts from a clean chassis. **(2) Empty-song auto-fill:** when you open Groovebox 8120 on an empty song (no real instruments yet), it creates all 8 instrument slots (rows 01–08) and loads the Paketti Default Instrument into each, so the groovebox is ready to play immediately. This happens *only* when you open 8120 — never on New Song — and it never touches a song that already has instruments. It is on by default and can be toggled off.

- Menu: `Main Menu:Tools:Paketti:Groovebox:Toggle Auto-Fill Default Instrument Slots (empty song)`
- Keybinding: `Global:Paketti:Groovebox 8120 Toggle Auto-Fill Default Slots`
- Preference: auto-fill default instrument slots on empty-song open (default ON)

### 2026-06-09 - Fix: nineteen more tools crash-proofed (SIGSEGV) on New Song / Load Song — the final batch

The third and final batch of the New Song / Load Song crash fix completes the sweep started with the Groovebox 8120, HyperEdit and Parameter Editor. These nineteen tools each attach a song observer when their dialog opens but only let go when the dialog is *closed* — so creating or loading a song while the dialog stayed open left an observer pointing at the song Renoise was destroying, which could crash. All nineteen now also tear down on `app_release_document_observable` (fires while the outgoing song is still valid), detaching their observers and closing their dialog before the song is released. In several of them this also fixed observers that were never being detached at all. This batch covers: **Command Wheel**, **Digitakt** (exporter), **Frame Calculator**, **Foreign Snippets** (sample visualizer), **Loaders** (Randomizer + auto-open device/sample-chain), **Metric Modulation** (both dialogs), **Mixer Parameter Exposer**, **Oldschool Slice Pitch** (BPM detect), **OpenMPT Linear Keyboard Layer**, **Pattern Delay Viewer**, **Pattern Length**, **Phrase Generator**, **Pitch Control** (BPM from sample), **Polyend Slice Switcher**, **Requests** (Convolver / VolDelayPan / Switch Note dialogs), **Stacker**, **Stem Slicer**, **Steppers**, and **WT Import**. With this, all 30 tools found by the audit — plus the original three — are crash-proofed against song changes.

### 2026-06-09 - Fix: four more tools crash-proofed (SIGSEGV) on New Song / Load Song — Automate Last Touched, Experimental Verify, Pattern Editor, Phrase Workflow

The second batch of the New Song / Load Song crash fix. These four tools already tried to handle song changes, but on the wrong moment: they cleaned up on "new document" (which fires *after* the old song is already gone) instead of on "document release" (which fires while the old song is still alive). That meant their song observers were still attached to the song Renoise was destroying. All four now detach on `app_release_document_observable` before the old song is freed, and re-attach to the new song afterwards — so they keep working across song changes without crashing. This batch covers: **Automate Last Touched**, **Experimental Verify** (playback monitor), **Pattern Editor** (edit-mode/track signaler), and **Phrase Workflow** (phrase-voice selection).

### 2026-06-09 - Fix: seven more tools crash-proofed (SIGSEGV) on New Song / Load Song — PCM Writer, PlayerPro Suite, PlayerPro Waveform Viewer, EQ30, Slice Effect Step Sequencer, Gater, Chords

A repo-wide audit found that the same New Song / Load Song crash fixed in the Groovebox 8120, HyperEdit, and Parameter Editor also lurked in seven more tools that keep a canvas or timer reading the song, or hold song observers, while their dialog stays open. Each could crash Renoise with a SIGSEGV in the song teardown if you created or loaded a song while the tool was open. All seven now tear everything down on `app_release_document_observable` — which fires while the outgoing song is still valid — detaching their observers, stopping their timers, and closing their dialog/canvas before the song is released. This batch (the highest-risk "Group A") covers: **PCM Writer**, **PlayerPro Suite**, **PlayerPro Waveform Viewer**, **EQ30 / EQ64**, **Slice Effect Step Sequencer**, **Gater**, and **Chords**. Two more batches (medium- and lower-risk tools) follow.

### 2026-06-09 - Fix: HyperEdit and Parameter Editor crash (SIGSEGV) on New Song / Load Song with the dialog or canvas open

Creating a new song or loading a song while **HyperEdit** or the **Parameter Editor** was open could crash Renoise with a SIGSEGV deep in the song/pattern-pool teardown — the same class of crash fixed for the Groovebox 8120 on 2026-06-08, now closed in the two remaining canvas tools. Both tools left their observers (HyperEdit: transport/track/device watchers; Parameter Editor: the selected-device watcher, the auto-open watcher, and per-parameter watchers) and their live canvas pointing at the song Renoise was about to destroy, because they only cleaned up when the dialog was *closed*. Both tools now also tear everything down on `app_release_document_observable` — which fires while the outgoing song is still valid — detaching every observer/timer and closing the dialog before the song is released. The Parameter Editor's auto-open watcher is reinstalled automatically on the newly loaded song. Reopen HyperEdit after the new song has loaded.

### 2026-06-08 - Feature: HyperEdit live MIDI Write — record knob tweaks into the playing step
...
Recording a sample into an 8120 row and then toggling beatsync — either the per-row "Beatsync per Instrument (00-7F sample)" checkbox or the **beatsync** switch in Renoise's Sample List — could do nothing, because the row's beatsync controls were bound to the wrong sample. The row picks its "primary" sample by looking for the one mapped across the full `00-7F` velocity range; if an empty placeholder slot still held that mapping, the controls watched the placeholder instead of the recorded take. Robustifications: (1) the primary-sample lookup now prefers a sample that actually carries audio (data-bearing + full-velocity, then any data-bearing slot, then any full-velocity slot, then slot 1) so the controls bind to the real recording, not an empty slot; (2) record-finalize re-points the per-row beatsync controls/observers at the recorded sample AND refreshes the row's sample-name label, so toggling beatsync in the Sample List reflects on the row and vice-versa, and the label stops showing "No sample available" after a take is recorded; (3) since Renoise commits the recorded sample asynchronously (the slot and its audio do not exist the instant recording stops — which left the label stuck on "No sample available" and the beatsync checkbox greyed out / unclickable), finalize now only acts once a data-bearing sample is actually present, and recording-stop polls every 100 ms (up to ~6 s) until the take appears, then maps it, refreshes the label, and activates the beatsync checkbox exactly once; (4) the row's sample-name label now resolves its sample through the same helper the beatsync controls use, so label and beatsync never disagree about which sample the row represents. No menu/keybinding/MIDI changes — behavior fix only.
...
### 2026-05-29 - Improvement: BeatSyncHack — batch injection + automatic restore after project reload

Two reliability upgrades to the BeatSyncHack save protection. (1) **Reload restoration:** the list of samples using extended (>512) BeatSyncLines is now stored inside the song as legal restoration metadata (`renoise.song().tool_data`, written during `app_will_save_document_observable`). When the project is reopened, the metadata is read and the extended values are re-applied automatically on a one-shot idle tick — previously, hacked values were lost on reload and had to be re-applied by hand. The illegal value itself is never written to a song property; only the recipe to rebuild it persists. (2) **Batch roundtrip:** restoration after save now groups hacked samples by instrument and re-injects each instrument in one XRNI save/reload instead of one reload per sample, and surfaces failures explicitly (`restored N, FAILED M — re-apply manually`) instead of leaving a sample silently clamped. A new global `pakettiBeatSyncHackRoundtrip(instrument_index, {[sample_index]=lines, ...})` is the shared core used by the dialog presets, the SlicePro extended-sync path, the after-save restore, and the after-reload restore. The save-time clamp-to-512 (which keeps the XRNS crash-safe) is unchanged.
