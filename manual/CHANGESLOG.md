...
### 2026-05-24 - Feature: Paketti Preferences — Reveal preferences.xml + Email to esaruoho

`Paketti0G01_Loader.lua` Folders row in the Paketti Preferences dialog now has two new buttons aimed at making it easy to send a preferences file when reporting an issue. **(1) "Reveal preferences.xml (for esaruoho@gmail.com)"** — calls `preferences:save_as("preferences.xml")` first to flush any in-memory edits, then `renoise.app():open_path(renoise.tool().bundle_path .. "preferences.xml")` to open the file in Finder/Explorer/Nautilus with the file selected. Status bar tells the user to drag it into a new email. **(2) "Email preferences.xml to esaruoho"** — does the same Reveal action, then opens a prefilled `mailto:esaruoho@gmail.com` URL with subject `Paketti preferences.xml` and a body asking the user to drag the file in and describe the issue. **Note on mailto attachments**: the `mailto:` URI scheme (RFC 6068) does not support file attachments, and every major mail client blocks any non-standard `attachment=` extensions for security — so this is the closest reliable cross-platform implementation: reveal the file, open a prefilled compose window, user drags the file in. **Note on the mailto: opener**: `renoise.app():open_url()` routes through the default browser (per the API docs: "Opens the default internet browser with the given URL"), which silently drops `mailto:` URLs. The button shells out via `os.execute` to the native opener — `open` on macOS, `start ""` on Windows, `xdg-open` on Linux — which is the only reliable way to make the OS hand the URL to the registered mail client.
