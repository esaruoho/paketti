...
Recording a sample into an 8120 row and then toggling beatsync — either the per-row "Beatsync per Instrument (00-7F sample)" checkbox or the **beatsync** switch in Renoise's Sample List — could do nothing, because the row's beatsync controls were bound to the wrong sample. The row picks its "primary" sample by looking for the one mapped across the full `00-7F` velocity range; if an empty placeholder slot still held that mapping, the controls watched the placeholder instead of the recorded take. Three robustifications: (1) the primary-sample lookup now prefers a sample that actually carries audio (data-bearing + full-velocity, then any data-bearing slot, then any full-velocity slot, then slot 1) so the controls bind to the real recording, not an empty slot; (2) record-finalize re-points the per-row beatsync controls/observers at the recorded sample, so toggling beatsync in the Sample List now reflects on the row and vice-versa; (3) finalize maps the recorded take by locating the highest-index data-bearing slot and runs a one-shot 150 ms retry, covering the case where Renoise commits the recorded sample a document tick after recording stops. No menu/keybinding/MIDI changes — behavior fix only.
