...
Fixed the brittle "Expose on Mixer" checkbox in the Selected Device Parameter Editor. Enabling it now immediately exposes the current device's automatable parameters from the editor's own device state, even when Renoise is not currently showing the Mixer frame, instead of waiting for a mixer-visible moment or a later drag. The expose path now also reports when the current editor target is not a track DSP device instead of silently swallowing the failure, and no longer crashes by comparing Renoise `AudioDevice` objects directly. Menu: `Mixer:Paketti Gadgets:Selected Device Parameter Editor...`.
