...
local function PCMWriterSelectAdjacentWaveform(direction)
  ...
  local next_index = popup.value + direction
  if next_index < 1 then
    next_index = #popup.items
  elseif next_index > #popup.items then
    next_index = 1
  end

  popup.value = next_index
  current_waveform_type = popup.items[next_index]
...
function PCMWriterLoad2RandomAKWFAndExport(skip_devices)
  ...
  if success_a and success_b then
    ...
    PCMWriterExportWaveAAndBToSample(skip_devices)
...
function PCMWriterLoad2RandomAKWFAndExportWithUnison()
  ...
  -- First call the regular Write AKWF A&B function with skip_devices flag
  PCMWriterLoad2RandomAKWFAndExport(true)
  ...
  (pcm_current_scene == 1) and vb:row{ -- SAMPLE_SETTINGS_ROW STARTS
    ...
    (pcm_current_scene == 1) and vb:row{ -- SHAPE_PARAMS_ROW STARTS
      ...
      vb:slider{
        ...
        width = 120,  -- Much larger slider
        steps = pakettiSteps(1, -1),  -- Step by 1 for proper integer increments
        notifier = function(value)
