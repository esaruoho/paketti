...
Esa then reported that the chromatic lift was gone, but the whole kit sounded at least one octave too low. Joshua identified the deeper root bug: `opts.base_key` won the `or` chain before the per-sample `key`, so every wave received the same root metadata. The exporter now checks `key` first and preserves each sample's real chromatic root. The temporary octave-fixed image remains historical evidence, not the recommended export.

Esa then reported that Cyclone showed `RX2_FUNK` loaded but only the first kick triggered. Inspection of the `.O01` showed the first `Splt` had no `Parm` key, as Typhoon format requires, and Paketti's group lower key was `0`; subsequent split keys were 37 onward. The first split therefore landed at C-0, not the requested C-2 base. `build_voice_groups` now sets the group lower key to the first split's actual key, so the no-Parm first split starts at the correct base key while subsequent slices remain consecutive.
