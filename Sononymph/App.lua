...
function App:__init(...)
  ...
  self.filemon.changed_observable:add_notifier(function()
    local selection = App.parse_config(self.preferences.SononymphPathToConfig.value)
    if selection then 
  ...
  self.preferences.SononymphPollingInterval:add_notifier(function()
    self.filemon.polling_interval = self.preferences.SononymphPollingInterval.value
  end)
  ...
  self.preferences.SononymphPathToExe:add_notifier(function()
    self:check_paths()
  end)
  self.preferences.SononymphPathToConfig:add_notifier(function()
    self:check_paths()
  end)
...
function App:start_monitoring()
  ...
  self.filemon.paths = {
    --self.preferences.SononymphPathToExe.value,
    self.preferences.SononymphPathToConfig.value,
  }
...
function App:toggle_live_transfer()
  ...
  if not self.live_transfer_observable.value 
    and self.preferences.SononymphShowTransferWarning.value 
  then 
--[[    local choice = renoise.app():show_prompt("Enable auto-transfer?",""
    ...
    elseif (choice == "Always (don't show warning)") then ]]--
      self.preferences.SononymphShowTransferWarning.value = false
...
function App:check_paths()
  ...
  local path = self.preferences.SononymphPathToExe.value
...
function App:do_transfer()
  ...
  local config_folder,_,__ = cFilesystem.get_path_parts(self.preferences.SononymphPathToConfig.value)
...
function App:do_search()
  ...
  local success,err = App.check_path(self.preferences.SononymphPathToExe.value)
  if not success then 
...
function App:test_config_parsing()
  ...
  local config_path = self.preferences.SononymphPathToConfig.value
...
function App:debug_versions()
  ...
  table.insert(debug_info, "\nCurrent config path: " .. self.preferences.SononymphPathToConfig.value)
  table.insert(debug_info, "Current config exists: " .. (io.exists(self.preferences.SononymphPathToConfig.value) and "YES" or "NO"))
...
function App:load_selected_sample_from_sononym(show_prompt)
  ...
  local current_selection = App.parse_config(self.preferences.SononymphPathToConfig.value)
...
-- Preferences are now handled by Paketti integration
...
-- flip_a_coin(renoise.tool().preferences.SononymphPathToConfig.value) -- Removed auto-call 