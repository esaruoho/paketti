...
function AppUI:__init(...)
  ...
  self.owner.preferences.SononymphPathToExe:add_notifier(function()
    self.update_requested = true
  end)
  
  self.owner.preferences.SononymphPathToConfig:add_notifier(function()
    self.update_requested = true
  end)
  
  self.owner.preferences.SononymphShowPrefs:add_notifier(function()
    self.update_requested = true
    self.prefs_toggle_enabled = self.owner.preferences.SononymphShowPrefs.value
  end)
...
function AppUI:show()
  ...
  if not self.owner.paths_are_valid then 
    local app_path = self.owner.preferences.SononymphPathToExe.value
    local config_path = self.owner.preferences.SononymphPathToConfig.value
  ...
  return vb:column{
    ...
    vb:column{
      ...
      vb:row{
        ...
        vb:button{
          bitmap = "./Sononymph/source/icons/detach.bmp",
      ...
      vb:row{
        ...
        vb:checkbox{
                      bind = self.owner.preferences.SononymphAutotransfercreatenew,
        },
        vb:text{
          text = "New Instrument",style="strong",font="bold",
        },
        vb:checkbox{
                      bind = self.owner.preferences.SononymphAutotransfercreateslot,
        },
        vb:text{
      ...
      vb:row{
        vb:button{
          ...
          width = 22,
          notifier = function()
                    self.owner.preferences.SononymphShowPrefs.value = not self.owner.preferences.SononymphShowPrefs.value
        self.prefs_toggle_enabled = self.owner.preferences.SononymphShowPrefs.value
    ...
    vb:column{    
      ...
      vb:row{
        ...
        vb:checkbox{
                      bind = self.owner.preferences.SononymphAutostart,
        },
        vb:text{
          text = "Autostart",
          style = "strong",
...
function AppUI:update()
  ...
  ctrl.width = AppUI.INPUT_W 
      ...
      local path_to_exe = self.owner.preferences.SononymphPathToExe.value 
  ctrl = vb.views["path_to_exe"]
  ctrl.text = path_to_exe
  ctrl.tooltip = ctrl.text
  ctrl.width = AppUI.INPUT_W
  
      local path_to_config = self.owner.preferences.SononymphPathToConfig.value 
  ...
  ctrl = vb.views["preferences_content"]
      ctrl.visible = self.owner.preferences.SononymphShowPrefs.value
