...
local dialog_content = vb:column{
  ...
  ", Josh Montgomery, Filthy Animal, AZ-Rotator, Roby, Onix4MAN AKA PinkCell, Pietro Bttns, Guiq, MGI, xpinvert, Defense Mechanism, tagbody " ..
  " and many others."},
...
function create_button_list()
  ...
  table.insert(buttons, {"Advanced Subdivision Calculator", "show_subdivision_calculator_dialog"})
  table.insert(buttons, {"Automation Curve Fill", "PakettiAutomationCurveFill"})
  table.insert(buttons, {"BPM Switcher", "PakettiBPMSwitcherDialog"})
  table.insert(buttons, {"BPM-Based Slice", "showBPMBasedSliceDialog"})
  table.insert(buttons, {"Category Management (MIDI)", "show_category_statistics"})
  table.insert(buttons, {"CCizer TXT->CC Loader", "PakettiCCizerLoader"})
  table.insert(buttons, {"CCizer TXT->MIDI Control Loader", "PakettiCreateMIDIControlFromTextFile"})
  ...
  table.insert(buttons, {"OctaCycle Generator", "PakettiOctaCycle"})
  table.insert(buttons, {"Octatrack .OT File Analysis", "PakettiOTDebugDialog"})
  table.insert(buttons, {"Offset Sample Buffer", "pakettiOffsetDialog"})
  ...
  table.insert(buttons, {"Paketti Arpeggiator", "PakettiArp_ShowDialog"})
  table.insert(buttons, {"Paketti Capture Last Take", "PakettiCaptureLastTakeDialog"})
  table.insert(buttons, {"Paketti Chords - Progression Player", "PakettiChords_CreateDialog"})
  ...
  table.insert(buttons, {"Paketti Golden Chord Library", "show_golden_chord_dialog"})
  table.insert(buttons, {"Paketti Golden Ratio Tempo/Rhythm", "apply_golden_tempo_relationship"})
  table.insert(buttons, {"Paketti Microtonal Tunings", "show_microtonal_tuning_dialog"})
  table.insert(buttons, {"Paketti MIDI Import", "show_midi_import_dialog"})
  table.insert(buttons, {"Paketti Minimize Cheatsheet", "pakettiMiniCheatsheetHorizontal"})
  table.insert(buttons, {"Paketti Randomize Delay Column", "PakettiFillRandomizeDelayDialog"})
  table.insert(buttons, {"Paketti Sectionizer", "PakettiSectionizer"})
  table.insert(buttons, {"Paketti Split Note into N Equal Pieces", "PakettiNoteSplitDialog"})
  table.insert(buttons, {"Paketti RePitch", "pakettiSimpleSampleTuningDialog"})
  table.insert(buttons, {"Paketti Deactivator", "PakettiDeactivatorDialog"})
  table.insert(buttons, {"Paketti XYPad Sample Rotator", "xypad"})
  ...
  table.insert(buttons, {"Plugin Details", "pakettiDebugPluginInfoDialog"})
  table.insert(buttons, {"Plugin Slots Configuration", "pakettiPluginSlotsDialog"})
  table.insert(buttons, {"Polyend Pattern Browser", "PakettiPolyendPatternBrowser"})
  ...
  table.insert(buttons, {"Set EditStep&Enter", "pakettiEditStepDialog"})
  table.insert(buttons, {"Set Pattern/Phrase Length", "pakettiLengthDialog"})
  ...
  table.insert(buttons, {"Sononymph", function() SononymphStart(true) end})
  ...
  -- "Track Mapping" removed: the dialog at PakettiPolyendPatternData.lua:1642 is a sub-dialog
  -- inside import_pattern_to_renoise(pattern_data, target_pattern_index, track_mapping, filepath)
  -- and requires real import args — it cannot be opened standalone. If a standalone version
  -- is wanted, a wrapper function needs to be written first.
...
function execute_dod_selection()
  if dod_selected_index > 0 and dod_selected_index <= #dod_filtered_buttons then
    ...
    -- Use rawget to bypass Renoise's strict global metatable — prevents
    -- "variable 'X' is not declared" errors if a function doesn't exist
    if type(func) == "function" then
      func()
    else
      local global_func = rawget(_G, func)
      if global_func then
        global_func()
      else
        renoise.app():show_warning("Dialog function '" .. tostring(func) .. "' is not available.\nThis feature may require a newer version of Renoise.")
      end
    end
